...
class LoginScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sharedPreferences = getSharedPreferences("com.example.proactive_opsc7311_poe", Context.MODE_PRIVATE)
    ...
    fun onForgotPasswordClicked(view: View) {
        ...
        val emailInput = EditText(this).apply {
            inputType = InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS  // Set the input type to accept email addresses.
        ...
        AlertDialog.Builder(this)
            .setTitle("Reset Password")  // Set the title of the dialog.
            ...
            .setPositiveButton("Send") { dialog, which ->  // Define a 'Send' button and its behavior.
                ...
                if (email.isNotEmpty()) {  // Check if the email field is not empty.
                    // Request Firebase to send a password reset email.
                    FirebaseAuth.getInstance().sendPasswordResetEmail(email)
                        .addOnCompleteListener { task ->
                            if (task.isSuccessful) {  // Check if the email was sent successfully.
                                Toast.makeText(this, "Reset instructions sent to your email", Toast.LENGTH_LONG).show()  // Show a success message.
                            } else {  // Handle the case where sending the email failed.
                                Toast.makeText(this, "Failed to send reset email", Toast.LENGTH_LONG).show()  // Show an error message.
                            }
                        }
                } else {  // If the email field is empty, prompt the user to fill it.
                    Toast.makeText(this, "Email field cannot be empty", Toast.LENGTH_LONG).show()
                }
            }
            .setNegativeButton("Cancel", null)  // Define a 'Cancel' button that simply dismisses the dialog.
